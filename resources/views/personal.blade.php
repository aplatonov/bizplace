    ...
    <div class="col-md-12">
        <div class="box">
            <div class="box-header">
                <form method="POST" action="{{ url('/personal/add') }}" id="addPersonalForm" style="display:none;">
                    {{ csrf_field() }}
                    <input type="hidden" name="ownerID" value="{{ Auth::user()->id }}" id="ownerID">
                </form>
                ...
                <div class="box-tools">
                    <div class="input-group input-group-sm" style="width: 250px;">
                        ...
                        <input class="form-control pull-right" name="searchTextVisible" id="searchTextVisible" placeholder="пока не работает" type="text" value="{{ Request::get('searchText') }}">
            ...
            <div class="box-body table-responsive no-padding">
                <table class="table table-striped table-hover table-condensed">
                    ...
                    <tr>
                        <th class="text-center"><a href="?page={{ $data['personal']->currentPage() }}&order=id&dir={{ $data['dir'] ? $data['dir'] : 'asc' }}">Код</a>{!! $data['page_appends']['order'] == 'id' ? $data['dir'] == 'desc' ? '<span class="glyphicon glyphicon-arrow-down"></span>' : '<span class="glyphicon glyphicon-arrow-up"></span>' : '' !!}</th>
                        <th><a href="?page={{ $data['personal']->currentPage() }}&order=object_name&dir={{ $data['dir'] ? $data['dir'] : 'asc' }}">Название</a>{!! $data['page_appends']['order'] == 'person_name' ? $data['dir'] == 'desc' ? '<span class="glyphicon glyphicon-arrow-down"></span>' : '<span class="glyphicon glyphicon-arrow-up"></span>' : '' !!}</th>
                        <th class="text-center"><a href="?page={{ $data['personal']->currentPage() }}&order=experience&dir={{ $data['dir'] ? $data['dir'] : 'asc' }}">Опыт, лет</a>{!! $data['page_appends']['order'] == 'experience' ? $data['dir'] == 'desc' ? '<span class="glyphicon glyphicon-arrow-down"></span>' : '<span class="glyphicon glyphicon-arrow-up"></span>' : '' !!}</th>
                        <th class="text-center"><a href="?page={{ $data['personal']->currentPage() }}&order=hour_rate&dir={{ $data['dir'] ? $data['dir'] : 'asc' }}">Ставка, р/час</a>{!! $data['page_appends']['order'] == 'hour_rate' ? $data['dir'] == 'desc' ? '<span class="glyphicon glyphicon-arrow-down"></span>' : '<span class="glyphicon glyphicon-arrow-up"></span>' : '' !!}</th>
                        <th class="text-center"><a href="?page={{ $data['personal']->currentPage() }}&order=speciality_id&dir={{ $data['dir'] ? $data['dir'] : 'asc' }}">Специальность</a>{!! $data['page_appends']['order'] == 'speciality_id' ? $data['dir'] == 'desc' ? '<span class="glyphicon glyphicon-arrow-down"></span>' : '<span class="glyphicon glyphicon-arrow-up"></span>' : '' !!}</th>
                    ...
                    @foreach($data['personal'] as $person)
                        <tr> 
                            ...
                            <td><a href="/personal/{{ $person->id }}">{{ $person->person_name }}</a><br>
                                <small>
